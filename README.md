...
The `serve` function returns a Promise that resolves once the server starts listening.
...
serve(argv, { config }).then((result) => {
  const { host, port } = result.options;
...
serve(argv, { config }).then((server) => {
  server.on('build-started', ({ server, options }) => {
